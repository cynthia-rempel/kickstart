...
cd cloud-init
genisoimage -output /var/tmp/media/atomic01-cidata.iso -volid cidata -joliet -rock user-data meta-data
cd ..
...
cp kickstart.cfg /var/tmp/media/mydrive/
cp isolinux.cfg /var/tmp/media/mydrive/isolinux/
...
mkisofs -r -T -J -V "CentOS Atomic Host 7 x86_64" \
